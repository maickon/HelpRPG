...
$pagina_nome = 'Idéias de aventura';
$descriacao= array('create'=>'Cadastrando uma idéia','edit'=>'Editando a idéia','destroy'=>'Apagando uma idéia');
...
switch ($tela):
    ...
    $tag->elemento('form','class="userForm" method="post" action="" enctype="multipart/form-data" ');
        $tag->elemento('fieldset');
			...
			$tag->elemento('label','for="titulo"');
				echo 'Título:';
			$tag->close('label');
			$tag->elemento('input','type="text" size="50" name="titulo" value=""');
			
			$tag->elemento('label');
				echo ('Texto da aventura:');
			$tag->close('label');
			$tag->elemento('br');
	...
	case 'listar':
		echo '<div align="right">';
			echo '<h2 align="left">';
				echo '<a href="?m='.$modulo.'&t=incluir" title="Novo cadastro"><img src="img/plus.png" alt="Novo cadastro" />Nova Aventura</a>';
			echo '</h2>';	
		echo '</div>';
		...
		if(isAdmin() == TRUE || $sessao->getVar('id_user') == $_GET['id']):
			if(isset($_GET['id'])):
				...
				if(isset($_POST['editar'])):
					...
					if($objeto->linhas_afetadas == 1):
						printMsg('Aventura editada com sucesso. <a href="?m='.$modulo.'&t=listar">Exibir aventura</a>');
						unset($_POST);
					else:
						printMsg('Nenhum dado foi alterado. <a href="?m='.$modulo.'&t=listar">Exibir aventura</a>','alerta');	
					endif;
				endif;				
		 ...
		 $tag->elemento('form','class="userForm" method="post" action="" enctype="multipart/form-data" ');
			$tag->elemento('fieldset');
				...
				$tag->elemento('label','for="titulo"');
					echo 'Título:';
				$tag->close('label');
				$tag->elemento('input','type="text" size="50" name="titulo" value="'.$objeto_resp->titulo.'"');
		...
		if(isAdmin() == TRUE || $sessao->getVar('id_user') == $_GET['id']):
			if(isset($_GET['id'])):
				...
				if(isset($_POST['excluir'])):
					...
					if($objeto->linhas_afetadas == 1):
						printMsg('Dados deletados com sucesso. <a href="?m='.$modulo.'&t=listar">Exibir aventura</a>');
						unset($_POST);
					else:
						printMsg('Nenhum dado foi deletado. <a href="?m='.$modulo.'&t=listar">Exibir aventura</a>','alerta');	
					endif;
				endif;
				$objeto_exibir = new $classe();
				$objeto_exibir->extras_select = " WHERE id=$id";
		 ...
		 $tag->elemento('form','class="userForm" method="post" action="" enctype="multipart/form-data" ');
			$tag->elemento('fieldset');
				...
				$tag->elemento('label','for="titulo"');
					echo 'Título:';
				$tag->close('label');
				$tag->elemento('input','type="text" size="50" name="titulo" disabled="disabled" value="'.limpar_campo_excluir('titulo',$objeto_resp).'"');
				...
				$tag->elemento('textarea','class="ckeditor" id="editor1" name="texto" disabled="disabled"');
					echo utf8_decode(limpar_campo_excluir('texto',$objeto_resp));
				$tag->close('textarea');
				$tag->elemento('br');
				$tag->elemento('div','class="small-2 large-12 columns" align="center"');
					$tag->elemento('input','type="button" onclick="location.href=\'?m='.$modulo.'&t=listar\'" value="Cancelar" class="small button "'); 
