...
$pagina_nome = 'Comentários Page';
$descriacao= array('create'=>'Cadastrando minha página de blog','edit'=>'Editando minha página de blog','destroy'=>'Apagando minha página de blog');
...
switch ($tela):
    ...
    $tag->elemento('form','class="userForm" method="post" action="" enctype="multipart/form-data" ');     
			...
			$tag->elemento('label');
				echo ('Comentário:');
			$tag->close('label');
			$tag->elemento('textarea',' name="texto"');
			$tag->close('textarea');
			
			$tag->elemento('input','type="button" onclick="location.href=\'?p=post&id='.$_GET['id'].'\'" value="Visualizar meu post" class="small button "'); 
			echo ' ';     
			$tag->elemento('input','type="submit" name="cadastrar" value="Comentar" class="small button "');     
		...
		$tag->close('tbody');
			...
			while($resp = $objeto->retorna_dados()):
				$tag->elemento('tr'); 
					...
					$tag->elemento('td','class="center"');
						printf('%s',date("d/m/Y"),strtotime($resp->data));
					$tag->close('td');
					
					$tag->elemento('td','class="center"');
						printf('%s',$resp->usuario);
					$tag->close('td');
		...
		<?php
		 $tag->elemento('form','class="userForm" method="post" action="" enctype="multipart/form-data" ');
			$tag->elemento('fieldset');
				...
				$tag->elemento('label','for="titulo"');
					echo 'Título:';
				$tag->close('label');
				$tag->elemento('input','type="text" size="50" name="titulo" autofocus="autofocus" value="'.limpar_campo_excluir('titulo',$objeto_resp).'"');
				
				$tag->elemento('label','for="categoria"');
					echo 'Categoria:';
				$tag->close('label');
				$tag->elemento('input','type="text" size="50" name="categoria" value="'.limpar_campo_excluir('categoria',$objeto_resp).'"');
		 ...
		 $tag->elemento('form','class="userForm" method="post" action="" enctype="multipart/form-data" ');
			$tag->elemento('fieldset');
				...
				$tag->elemento('label','for="titulo"');
					echo 'Título:';
				$tag->close('label');
				$tag->elemento('input','type="text" size="50" name="titulo" disabled="disabled" autofocus="autofocus" value="'.limpar_campo_excluir('titulo',$objeto_resp).'"');
				
				$tag->elemento('label','for="categoria"');
					echo 'Categoria:';
				$tag->close('label');
				$tag->elemento('input','type="text" size="50" name="categoria" disabled="disabled" value="'.limpar_campo_excluir('categoria',$objeto_resp).'"');
